...
async function loadWeather() {
  ...
  try {
    ...
    const [forecastItems, nowcastItems, airQuality] = await Promise.all([
      fetchKmaVilageForecast(),
      fetchKmaUltraNowcast().catch(() => []),
      fetchAirKoreaRealtime()
    ]);

    dayData = buildDailyDataFromKma(forecastItems, nowcastItems, airQuality.pm25);
    if (!dayData.length) throw new Error('일별 데이터 없음');

    setStatus('기상청·환경공단 데이터 연결됨');
    setLoading(`세종시 ${SEJONG.airStation} 측정소 기준 최신 데이터를 반영했습니다.`);
  } catch (error) {
    console.warn(error);
    dayData = FALLBACK_DAYS;
    setStatus('예시 데이터 사용 중');
    setLoading('API 키나 응답을 확인해야 합니다. 현재는 예시 데이터로 표시합니다.');
  }
...
async function fetchKmaVilageForecast() {
  const base = getKmaVilageBaseDateTime();
  ...
  return readPublicDataItems(json, '기상청 단기예보');
}
...
function readPublicDataItems(json, label) {
  ...
  if (resultCode && resultCode !== '00') {
    throw new Error(`${label} 오류: ${header?.resultMsg || resultCode}`);
  }
...
function getKmaVilageBaseDateTime() {
  const now = new Date();
  const korea = new Date(now.toLocaleString('en-US', { timeZone: 'Asia/Seoul' }));
  korea.setMinutes(korea.getMinutes() - 40);

  const baseTimes = ['0200', '0500', '0800', '1100', '1400', '1700', '2000', '2300'];
  const current = korea.getHours() * 100 + korea.getMinutes();
  let selected = baseTimes.filter(time => Number(time) <= current).pop();
  ...
  return { date: formatKmaDate(korea), time: selected };
}
