...
function renderSelectedDay() {
  ...
  updateWeatherBackground(selected);
...
function updateWeatherBackground(day) {
  const condition = getWeatherCondition(day.code, day.rain);
  const night = isNightForSelectedDate(day.date);
  const theme = `${condition}-${night ? 'night' : 'day'}`;
  document.body.dataset.theme = theme;
}

function getWeatherCondition(code, rain) {
  if (rain > 1.5 || code >= 60) return 'rain';
  if ((code >= 1 && code <= 3) || (code >= 45 && code < 60)) return 'cloud';
  return 'clear';
}

function isNightForSelectedDate(dateString) {
